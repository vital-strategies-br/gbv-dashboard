...
function AxisGridTicks({ axis, limits, numTicks = 5, axisLength, otherLength, startMargin, axisOffset = 0, showLines = false, roundSteps = false }: AxisGridTicksProps) {
    const tickPositions = linspace(limits[0], .9 * limits[1], numTicks, roundSteps);

    return (
        <g className={`${axis}-grid-ticks`}>
            {tickPositions.map((value) => {
                const lengthPercentage = (value - limits[0]) / limits[1];
                const lengthInPixels = (lengthPercentage * (axisLength - axisOffset)) + axisOffset;
                let x1: number, y1: number, x2: number, y2: number, textOffsetX: number, textOffsetY: number, anchor: string;
                ...
                return (
                    <g key={value}>
                        ...
                        <text x={x1 + textOffsetX} y={y1 + textOffsetY} textAnchor={anchor}>
                            {roundSteps ? value : value.toFixed(2)}
                        </text>
                    </g>
                );
            })}
    ...
    return (
        ...
        {binCounts.map((count, index) => (
          ...
          />
            ...
            <AxisGridTicks axis="x" limits={[0, MAX_VALUE]} axisLength={chartAreaWidth} otherLength={chartAreaHeight} startMargin={chartMarginBottom} axisOffset={16 + 32 + 48} />
            <AxisGridTicks axis="y" limits={YLIM} axisLength={chartAreaHeight} otherLength={chartAreaWidth} startMargin={chartMarginLeft} showLines roundSteps />
            <g className="barchart-bar-na">
                <rect x={16 + chartMarginLeft} y={chartAreaHeight - naBarHeight} width="32" height={naBarHeight} />
                <text x={16 + chartMarginLeft + 16} y={chartAreaHeight + 30} textAnchor="middle">n.d.</text>
            </g>
            <text
